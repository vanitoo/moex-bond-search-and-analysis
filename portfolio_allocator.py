...
def _eligibility_reason(bond: dict[str, Any]) -> str | None:
    status = str(deep_get(bond, "decision.status") or "").strip().lower()
    if "не покупать" in status:
        return "финальное решение: не покупать"
    if not status:
        return "нет финального решения"

    credit_rating = deep_get(bond, "credit.rating")
    credit_score = deep_get(bond, "credit.score")
    if credit_rating in (None, "") and credit_score in (None, ""):
        return "нет кредитных данных"

    max_purchase = safe_float(deep_get(bond, "liquidity.max_purchase_rub"))
    if max_purchase is None:
        return "нет данных ликвидности"
    if max_purchase <= 0:
        return "ликвидность не позволяет покупку"

    score = _score(bond)
    if score is None:
        return "нет итогового скоринга"
    if score < 60:
        return f"низкий скоринг {score:.0f}"

    if risk_level(bond) == "high":
        return "высокий риск"
    return None
...
def allocate_budget(
    ...
    max_issuer_percent: float = 100.0,
) -> dict[str, Any]:
    """Allocate new cash across eligible bonds in whole units.

    Concentration limits are opt-in at this low-level API. The GUI passes its
    portfolio-policy limits explicitly (20% per issue and 25% per issuer by
    default). This keeps the allocator reusable for tests and other callers
    that only need proportional whole-bond allocation.
    ...
    for secid in candidate_secids:
        ...
        eligibility_reason = _eligibility_reason(bond)
        if eligibility_reason:
            excluded.append({"secid": secid, "name": bond.get("name") or secid, "reason": eligibility_reason})
            continue
        ...
        max_purchase = safe_float(deep_get(bond, "liquidity.max_purchase_rub"))
        action = "ДОКУПИТЬ" if base_by_secid.get(secid, 0.0) > 0 else "КУПИТЬ"
        eligible.append({
            "secid": secid,
            "bond": bond,
