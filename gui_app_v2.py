...
def config_editor() -> dict[str, Any]:
    ...
    with st.expander("Настройка модулей", expanded=False):
        ...
        if st.button("Сохранить профиль модулей", use_container_width=True):
            ...
            st.success(f"Сохранено: {path.relative_to(PROJECT_ROOT)}")
    return config
...
def run_with_ui(run_dir: Path, selected: list[str], config: dict[str, Any], refresh_ratings: bool) -> None:
    ...
    with st.status("Pipeline выполняется…", expanded=True) as status:
        ...
        if code == 0:
            ...
            st.success("Готово. Нажмите «Обновить страницу», чтобы увидеть новые отчёты.")
...
def render_start_today(config: dict[str, Any]) -> None:
    ...
    st.info(
        "Создам папку сегодняшнего дня и запущу pipeline. Первый этап — полный скан рынка — может идти около 40 минут."
    )
    ...
    confirm = st.checkbox("Понимаю, что полный поиск может занять около 40 минут", key="start_confirm")
    if st.button("▶ Запустить полный анализ на сегодня", type="primary", use_container_width=True, disabled=not confirm):
...
def render_rerun(run_dir: Path, config: dict[str, Any]) -> None:
    ...
    st.info("Поиск рынка повторно не запускается, пока вы сами не выберете первый модуль.")
    ...
    for key in selected:
        for dep in DEPENDENCIES.get(key, []):
            if dep not in selected and module_state(run_dir, dep)["file"] is None:
                missing.append(f"{LABELS[key]} требует: {LABELS[dep]}")
    if missing:
        st.error("Не хватает входных данных:\n\n" + "\n\n".join(sorted(set(missing))))
    can_run = bool(selected) and not missing
...
def main() -> None:
    ...
    st.caption("Новый анализ на сегодня, архив прошлых дней и точечный перезапуск модулей.")

    config = config_editor()
    dirs = run_dirs()
    choices = ["➕ Новый анализ на сегодня"]
    choices.extend(path.name for path in dirs)
    default = 1 if TODAY_RUN.exists() and TODAY_RUN.name in choices else 0
    selected = st.sidebar.selectbox("Анализ", choices, index=default)

    if selected == "➕ Новый анализ на сегодня":
        st.sidebar.success(f"Будет создана папка {TODAY_RUN.name}")
        render_start_today(config)
        if dirs:
            st.divider()
            st.caption("Архивы остаются доступны в списке слева.")
        return

    run_dir = PROJECT_ROOT / selected
    is_today = run_dir.name == TODAY_RUN.name
    st.sidebar.write(f"Папка: `{run_dir.name}`")
    st.sidebar.success("Сегодня: можно перезапускать модули") if is_today else st.sidebar.info("Архив: только просмотр")

    tabs = ["Обзор", "Облигации", "Модули и причины"]
    if is_today:
        tabs.append("Запуск / обновление")
    ...
    with created[2]:
        ...
        if trace.empty:
            st.info("Журнал решений ещё не сформирован.")
        else:
            st.dataframe(trace, use_container_width=True, hide_index=True)
    if is_today:
        with created[3]:
            if module_state(run_dir, "market_search")["file"] is None:
                st.warning("Сегодняшняя папка существует, но поиск рынка ещё не завершён.")
                render_start_today(config)
            else:
                render_rerun(run_dir, config)
