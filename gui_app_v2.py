...
MODULES = [
    ("market_search", "1. Поиск облигаций", "Полный скан рынка, обычно около 40 минут"),
...
def module_state(run_dir: Path, key: str) -> dict[str, Any]:
    if not run_dir.exists(): return {"status": "Нет результата", "updated": "—", "file": None}
    ...
    if path is None: return {"status": "Нет результата", "updated": "—", "file": None}
...
def save_gui_config(config: dict[str, Any]) -> Path:
    path = PROJECT_ROOT / "configs" / "gui_active.json"; path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(config, ensure_ascii=False, indent=2), encoding="utf-8"); return path


def config_editor() -> dict[str, Any]:
    active = PROJECT_ROOT / "configs" / "gui_active.json"; source = active if active.exists() else DEFAULT_CONFIG
    config = load_json(source, {"strategy": "balanced", "modules": {}}); modules = config.setdefault("modules", {})
...
def run_with_ui(run_dir: Path, selected: list[str], config: dict[str, Any], refresh_ratings: bool) -> None:
    ...
    with st.status("Pipeline выполняется…", expanded=True) as status:
        ...
        log_path = run_dir / "gui_last_run.log"; log_path.write_text(log, encoding="utf-8")
        if code == 0:
            status.update(label="Анализ успешно завершён", state="complete"); st.success("Готово. Обновите страницу.")
            if st.button("Обновить страницу", type="primary"): st.rerun()
        else:
            status.update(label=f"Ошибка выполнения, код {code}", state="error"); st.error(f"Лог сохранён: {log_path}")
...
def read_excel_safely(path: Path | None, preferred: list[str] | None = None) -> pd.DataFrame:
    ...
    try:
        book = pd.ExcelFile(path); sheet = next((x for x in (preferred or []) if x in book.sheet_names), book.sheet_names[0])
        return pd.read_excel(path, sheet_name=sheet)
    except Exception as exc:
        st.warning(f"Не удалось прочитать {path.name}: {exc}"); return pd.DataFrame()
...
def render_overview(run_dir: Path) -> None:
    ...
    frame = pd.DataFrame(states).drop(columns=["file"]); complete = int((frame["status"] == "Готово").sum())
    ...
    a, b, c = st.columns(3); a.metric("Модулей готово", f"{complete}/{len(MODULES)}"); b.metric("Бумаг в решениях", len(decisions)); c.metric("Допущено", candidates)
...
def render_bonds(run_dir: Path) -> None:
    ...
    if data.empty: st.info("Таблица облигаций ещё не сформирована."); return
    query = st.text_input("Поиск по названию или SECID"); view = data.copy()
    if query:
        mask = pd.Series(False, index=view.index)
        for column in ["Полное наименование", "Код ценной бумаги"]:
            if column in view.columns: mask |= view[column].astype(str).str.contains(query, case=False, na=False)
        view = view[mask]
...
def render_rerun(run_dir: Path, config: dict[str, Any]) -> None:
    ...
    for key in selected:
        for dep in DEPENDENCIES.get(key, []):
            if dep not in enabled: continue
    ...
    if missing: st.error("Не хватает входных данных:\n\n" + "\n\n".join(sorted(set(missing))))
...
def main() -> None:
    ...
    st.title("📊 MOEX Bond Lab"); st.caption("Финальное решение при любом наборе включённых модулей.")
    config = config_editor(); dirs = run_dirs(); choices = ["➕ Новый анализ на сегодня"] + [path.name for path in dirs]
    ...
    if selected == "➕ Новый анализ на сегодня": render_start_today(config); return
    run_dir = PROJECT_ROOT / selected; is_today = run_dir.name == TODAY_RUN.name
    ...
    with tabs[0]: render_overview(run_dir)
    with tabs[1]: render_bonds(run_dir)
    with tabs[2]:
        trace = trace_table(run_dir); st.dataframe(trace, use_container_width=True, hide_index=True) if not trace.empty else st.info("Журнал пока отсутствует")
    with tabs[3]: render_rerun(run_dir, config) if is_today else st.info("Архив доступен только для просмотра")
